...
test.describe("Step 25 — Planned bank & Plan B per day", () => {
  ...
  test("1. Locations shows Planned badge, sort, and unplanned filter", async ({ page }) => {
    ...
    const caneva = page.locator('[data-attraction-name="CanevaWorld Aqua Paradise"]');

    await expect(gardaland.getByTestId("bank-planned-badge")).toBeVisible({ timeout: 15000 });
    await expect(caneva.getByTestId("bank-planned-badge")).toHaveCount(0);
    ...
    await expect(caneva).toBeVisible();
    await expect(gardaland).toBeVisible();

    const canevaBox = await caneva.boundingBox();
    const gardalandBox = await gardaland.boundingBox();
    expect(canevaBox && gardalandBox && canevaBox.y < gardalandBox.y).toBeTruthy();
    ...
    expect(
      unplannedHeaderBox &&
        ...
        canevaBox &&
        gardalandBox &&
        unplannedHeaderBox.y < canevaBox.y &&
        canevaBox.y < plannedHeaderBox.y &&
  ...
  test("2. Itinerary Day 3 Plan B adds backup activity", async ({ page }) => {
    ...
    await expect(page.getByTestId("plan-b-option-bank-caneva-aqua")).toBeVisible();

    await page.getByTestId("plan-b-add-day-3-bank-caneva-aqua").click();
    await expect(
      page.locator("#day-card-3").getByText("CanevaWorld Aqua Paradise", { exact: true })
  ...
  test("3. Planned row navigates to itinerary day anchor", async ({ page }) => {
    ...
    await expect(page).toHaveURL(/\/itinerary#day-card-6/, { timeout: 15000 });
    await expect(page.locator("#day-card-6")).toBeVisible();
  });
});
